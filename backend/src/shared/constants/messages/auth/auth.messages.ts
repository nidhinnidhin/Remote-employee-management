export const AUTH_MESSAGES = {
    USER_NOT_FOUND: 'User not found',
    USER_NOT_ACTIVE: 'User is not active',
    INVALID_CREDENTIALS: 'Invalid credentials',
    ACCOUNT_NOT_VERIFIED: 'Account not verified. Please verify OTP.',
    MISSING_COMPANY_ID: 'User data incomplete (missing companyId). Please contact support or register again.',
    INVALID_REFRESH_TOKEN: 'Invalid refresh token',
    EXPIRED_REFRESH_TOKEN: 'Refresh token expired',
    ACCOUNT_NOT_ACTIVE: 'Account is not active',
    COMPANY_ALREADY_EXIST: 'Company already exists',
    USER_ALEADY_VERIFIED: 'User already verified',
    PASSWORD_RESET_SUCCESS: 'Password reset successful',
    MISSING_REFRESH_TOKEN: 'Refresh token missing',
    USER_ALREADY_EXIST: 'User already exist'
};