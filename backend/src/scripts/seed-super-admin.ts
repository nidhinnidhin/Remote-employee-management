...
async function bootstrap() {
    ...
    if (!email || !password) {
        console.error('SUPER_ADMIN_EMAIL or SUPER_ADMIN_PASSWORD not defined in .env');
        process.exit(1);
    }
    ...
    if (existingUser) {
        ...
        if (userRole !== UserRole.SUPER_ADMIN) {
            console.log(`User with email ${email} exists but is not SUPER_ADMIN. Updating role...`);
            ...
            console.log('Super Admin user already exists. Updating password to ensure match...');
            const salt = await bcrypt.genSalt(10);
            const hash = await bcrypt.hash(password, salt);
            existingUser.set('passwordHash', hash);
            await existingUser.save();
