// src/modules/meeting/application/use-cases/start-meeting.usecase.ts
import { Injectable, Inject, NotFoundException, UnauthorizedException } from '@nestjs/common';
import { IStartMeetingUseCase } from '../interfaces/meeting-use-cases.interface';
...
export class StartMeetingUseCase implements IStartMeetingUseCase {
  ...
  async execute(meetingId: string, companyId: string, userId: string): Promise<MeetingResponseDto> {
    ...
    if (meeting.creatorId !== userId) throw new UnauthorizedException('Only creator can start the meeting');

    const updated = await this._meetingRepository.updateStatus(meetingId, MeetingStatus.ONGOING);
