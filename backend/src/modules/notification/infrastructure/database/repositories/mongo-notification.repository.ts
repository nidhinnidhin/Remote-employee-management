...
export class MongoNotificationRepository implements INotificationRepository {
  ...
  async findById(id: string, companyId: string): Promise<NotificationEntity | null> {
    const doc = await this.notificationModel.findOne({ _id: id, companyId }).exec();
    return doc ? this.mapToEntity(doc) : null;
  }

  async findByUser(userId: string, companyId: string): Promise<NotificationEntity[]> {
  ...
  async markAsRead(id: string, companyId: string): Promise<NotificationEntity | null> {
  ...
  async delete(id: string, companyId: string): Promise<boolean> {
    const result = await this.notificationModel.deleteOne({ _id: id, companyId }).exec();
