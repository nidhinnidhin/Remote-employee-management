...
import { IDecideBreakRequestUseCase } from '../interfaces/attendance-use-cases.interface';
...
@Injectable()
export class DecideBreakRequestUseCase implements IDecideBreakRequestUseCase {
  constructor(
  ...
  async execute(
    ...
    if (attendance.pendingBreakRequest?.status !== 'PENDING') {
    ...
    const updatedAttendance = new AttendanceEntity(
      ...
      attendance.earlyOutApprovalStatus,
      attendance.earlyOutAdminRemarks,
      {
