    ...
    <div className="flex flex-col gap-6 p-6 animate-in fade-in duration-700 h-full overflow-y-auto">
      ...
      {meetings.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white/[0.01] border border-dashed border-white/[0.05] rounded-3xl mt-10">
          ...
          <h2 className="text-sm font-black text-white uppercase tracking-widest">No Meetings</h2>
          <p className="text-xs text-slate-500 mt-2">Create an instant meeting from your project sprint.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {meetings.map((meeting) => (
            <div key={meeting.id} className="group relative bg-[#0f1115] border border-white/[0.05] rounded-2xl p-6 hover:border-indigo-500/30 hover:shadow-2xl hover:shadow-indigo-500/5 transition-all">
              <div className="flex items-start justify-between mb-4">
                <div className={cn(
                  "px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest border",
                  meeting.status === 'ONGOING' ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" :
                  meeting.status === 'SCHEDULED' ? "bg-blue-500/10 text-blue-500 border-blue-500/20" :
              ...
              <h3 className="text-sm font-bold text-white mb-2">
                {meeting.type === 'INSTANT' ? 'Instant Session' : 'Scheduled Meeting'}
              </h3>
              ...
              <Button
                ...
                className={cn(
                  ...
                  meeting.status === 'ONGOING' ? "bg-emerald-500 hover:bg-emerald-600 text-black shadow-lg shadow-emerald-500/20" :
                  meeting.status === 'SCHEDULED' ? "bg-blue-500 hover:bg-blue-600 text-white shadow-lg shadow-blue-500/20" :
