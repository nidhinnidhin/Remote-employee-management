  ...
  const fetchMeetings = async (page: number) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        // Safe Extraction: Handles standard paginated backend schema or legacy arrays
        ...
        // Keep explicit pagination metadata properties structural sync
        if (result.data.meta) {
      ...
      console.error("Frontend admin meeting parsing crash:", err);
      setMeetings([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AdminLayoutWrapper>
      <div className="flex flex-col gap-6 p-6 animate-in fade-in duration-700 h-full overflow-y-auto justify-between">
        ...
        {/* Unified Pagination Segment Placement */}
