...
import { Plus, X, ListPlus, User, Star, ListChecks } from "lucide-react";
import BaseModal from "@/components/ui/BaseModal";
import FormInput from "@/components/ui/FormInput";
import FormDropdown from "@/components/ui/FormDropdown";
import Button from "@/components/ui/Button";
import { StoryPriority, StoryPoints, StoryStatus, CreateStoryPayload } from "@/shared/types/company/projects/user-story.type";
...
const CreateStoryModal: React.FC<CreateStoryModalProps> = ({
  ...
  const [formData, setFormData] = useState<Omit<CreateStoryPayload, 'projectId'>>({
    title: "",
    description: "",
    priority: "Medium",
    storyPoints: 5,
  ...
  const addCriterion = () => {
    ...
    if (formData.acceptanceCriteria.length >= 20) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      toast.error(result.error || "Failed to create user story");
    }
  };
  ...
  return (
    <BaseModal
      ...
      description="Define a new piece of work for the project."
      maxWidth="max-w-2xl"
    >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-muted uppercase">
               {formData.acceptanceCriteria.length} / 20
            </span>
          </div>
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="User can enter their email address..."
...
// Internal utility since Lucide doesn't have CheckPlus
