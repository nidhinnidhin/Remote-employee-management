...
interface FormDropdownProps {
  ...
  variant?: "emerald" | "accent"; // Safe theme toggle
}
...
const FormDropdown: React.FC<FormDropdownProps> = ({
  ...
  className,
  icon,
  variant = "emerald", // Default remains green for Employee side
}) => {
  ...
  return (
    <div className={cn("mb-0 relative group/dropdown", className)} ref={dropdownRef}>
      <label className={cn(
        ...
        isEmerald ? "group-focus-within/dropdown:text-emerald-500" : "group-focus-within/dropdown:text-blue-400"
      )}>
        {label} {required && <span className={isEmerald ? "text-emerald-500" : "text-blue-500"}>*</span>}
      </label>
      ...
      <button
        ...
        className={cn(
          ...
          isOpen && (isEmerald ? "border-emerald-500/50 ring-2 ring-emerald-500/10" : "border-blue-500/50 ring-2 ring-blue-500/10"),
          error && "border-red-900/50 bg-red-950/10"
        )}
      >
        <div className="flex items-center gap-3">
          {icon && <div className={cn("transition-colors", isEmerald ? "text-emerald-800" : "text-slate-400")}>{icon}</div>}
          ...
          {formattedOptions.map((option) => (
            <button
              ...
              className={cn(
                ...
                value === option.value 
                  ? (isEmerald ? "bg-emerald-500/10 text-emerald-400" : "bg-blue-500/10 text-blue-400") 
                  : "text-slate-400 hover:bg-white/5 hover:text-white"
              )}
            >
              <span>{option.label}</span>
              {value === option.value && (
                <div className={cn("w-1.5 h-1.5 rounded-full", isEmerald ? "bg-emerald-500 shadow-[0_0_10px_#10b981]" : "bg-blue-500 shadow-[0_0_10px_#3b82f6]")} />
              )}
            </button>
          ))}
        </div>
