...
export const PlanCard: React.FC<PlanCardProps> = ({
  ...
  return (
    <div className="group relative flex flex-col p-6 rounded-2xl  border border-[#2A2A2A] transition-all duration-300 hover:border-[rgb(var(--color-accent))]/50 hover:shadow-[0_0_20px_rgba(var(--color-accent),0.1)]">
      {/* Plan Name & Price */}
        ...
        {features.map((feature, index) => (
          <div key={index} className="flex items-center gap-3">
            <div className="flex-shrink-0 w-5 h-5 rounded-full bg-[rgb(var(--color-accent))]/10 flex items-center justify-center">
              <Check
                className="w-3 h-3 text-[rgb(var(--color-accent))]"
                strokeWidth={3}
              />
            </div>
