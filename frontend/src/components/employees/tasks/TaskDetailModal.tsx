...
import { Clock, Calendar, Hash, AlignLeft, Layers, CheckCircle2, MessageSquare, Send, CornerDownRight, Loader2 } from "lucide-react";
...
export default function TaskDetailModal({ task, isOpen, onClose }: TaskDetailModalProps) {
  ...
  // Reset tab & form when modal closes
  useEffect(() => {
    if (!isOpen) {
    ...
    >
        ...
        >
          ...
          {comments.length > 0 && (
            <span className={cn(
              ...
              activeTab === "comments" ? "bg-[#08090a]/30" : "bg-accent/20 text-accent"
            )}>
              {comments.length}
            </span>
          )}
      ...
      {activeTab === "comments" && (
        <div className="flex flex-col" style={{ height: "420px" }}>
          ...
          {/* Input */}
          <div className="pt-4 border-t border-white/[0.08] mt-4 shrink-0">
            {replyTo && (
