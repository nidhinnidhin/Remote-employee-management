...
import {
  Comment,
  CommentEntityType,
} from "@/shared/types/company/projects/comment.type";
import {
  getCommentsAction,
  addCommentAction,
...
export default function TaskDetailModal({
  ...
  // Instantly forces scroll container directly to the bottom boundary maximum
  ...
  const fetchComments = useCallback(
    async (skipGlobalLoading = false) => {
      if (!task) return;
      if (!skipGlobalLoading) setCommentsLoading(true);
      ...
      if (!skipGlobalLoading) setCommentsLoading(false);
    },
    [task],
  );

  // Handle data pre-fetch and instant scroll positioning when switching to the Comments tab
  useEffect(() => {
    if (isOpen && activeTab === "comments") {
      fetchComments(false).then(() => {
        requestAnimationFrame(() => {
          ...
          setTimeout(scrollToBottom, 180); // Defeats transition animation lags
        });
      });
    }
  }, [isOpen, activeTab, fetchComments, scrollToBottom]);
  ...
  // MutationObserver layout tracker: Snaps down instantly the exact millisecond any change happens
  useEffect(() => {
    if (!isOpen || activeTab !== "comments" || !scrollContainerRef.current)
      return;
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(e.target.files).forEach((file) => {
      ...
      setAttachments((prev) => [
        ...prev,
        { name: file.name, file, previewUrl },
      ]);
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      // Load background data changes cleanly without breaking layout trees
  ...
  const detailItems = [
    {
      label: "Estimated",
      value: `${task.estimatedHours || 0} hrs`,
      icon: Clock,
    },
    {
      label: "Auto Time",
      value: `${calculateWorkedHours()} hrs`,
      icon: CheckCircle2,
    },
    {
      label: "Manual Log",
      value: `${task.actualHours || 0} hrs`,
      icon: AlignLeft,
    },
    {
      label: "Due Date",
          ...
          return (
            <div
              ...
              onClick={() => {
                ...
                win?.document.write(
                  `<iframe src="${url}" style="border:0; top:0; left:0; bottom:0; right:0; width:100%; height:100%;" allowfullscreen></iframe>`,
                );
              }}
      ...
      <div className="flex items-center gap-1 p-1 rounded-xl bg-white/[0.03] border border-white/[0.06] mb-5 mt-2">
        <button
          ...
          className={cn(
            ...
            activeTab === "details"
              ? "bg-accent text-[#08090a] shadow-md"
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {detailItems.map((item) => (
              <div
                key={item.label}
                className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.05] flex items-center gap-3"
              >
      ...
      {activeTab === "comments" && (
        ...
        >
          ...
          <div
            ...
            className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar pb-4"
          >
            {commentsLoading ? (
              ...
              parentComments.map((parent) => (
                <div key={parent.id || parent._id} className="space-y-2">
                  ...
                  {childComments
                    .filter(
                      (child) => child.parentId === (parent.id || parent._id),
                    )
                    .map((child) => (
                      <div
                      ...
                      >
                        <div className="flex justify-between items-start mb-1.5">
                          <span className="text-xs font-bold text-slate-400">
                            {child.authorName ||
                              `User ${child.authorId.slice(-4)}`}
                          </span>
                          <span className="text-[10px] text-slate-500">
                            {formatCommentDate(child.createdAt)}
                          </span>
                        </div>
          ...
          {/* Form remains completely constant/fixed at the absolute bottom */}
          <div className="shrink-0 pt-4 border-t border-white/[0.08]  mt-auto space-y-3 sticky bottom-0 z-10 w-full">
            {replyTo && (
