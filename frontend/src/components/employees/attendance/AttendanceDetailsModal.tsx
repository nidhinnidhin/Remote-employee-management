...
export const AttendanceDetailsModal: React.FC<AttendanceDetailsModalProps> = ({
  ...
  // Pre-compiled action footer layout block
  ...
  return (
    <BaseModal
      isOpen={!!selectedLog}
      onClose={onClose}
      title="Swipe Chronology" // Sent as primitive string to satisfy BaseModalProps rules safely
      description={selectedLog?.date || ""}
    ...
    >
      {selectedLog && (
        <div className="space-y-6 relative text-left">
          {/* Absolute manual escape cross matching BaseModal structural container offsets */}
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-white/[0.01] border border-white/[0.04]">
            <div>
              <p className="text-[9px] font-black uppercase tracking-wider text-slate-400">Work Hours</p>
              <p className="text-sm font-black text-accent mt-0.5">
                {formatMinutesToHours(selectedLog.totalWorkMinutes)}
              </p>
            </div>
            <div>
              <p className="text-[9px] font-black uppercase tracking-wider text-slate-400">Break Limit</p>
              ...
              {selectedLog.activities.map((act, idx) => {
                ...
                return (
                  <div key={idx} className="relative flex items-start justify-between text-xs gap-3">
                    {/* Anchor dot layer rendering perfectly over contextual CSS variable colors */}
                    <div
                      className={cn(
