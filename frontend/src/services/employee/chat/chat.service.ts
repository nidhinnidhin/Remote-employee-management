...
export const chatService = {
  getConversations: async () => {
    const response = await api.get<Conversation[]>(
      API_ROUTES.COMPANY.CHATS.CONVERSATIONS,
    );
  ...
  searchEmployees: async (query: string) => {
    const response = await api.get<any[]>(
      `${API_ROUTES.COMPANY.EMPLOYEES.BASE}?search=${query}`,
    );
    return response.data;
  },
  ...
  updateConversation: async (
    ...
    const response = await api.patch<Conversation>(
      `${API_ROUTES.COMPANY.CHATS.CONVERSATIONS}/${id}`,
      dto,
    );
    return response.data;
  },
  ...
  uploadGroupImage: async (file: File) => {
    ...
    const response = await api.post<{ imageUrl: string }>(
      `${API_ROUTES.COMPANY.CHATS.CONVERSATIONS.replace("conversations", "upload-image")}`,
      formData,
      {
  ...
  // 🔹 ADDED TO CONSUME THE MULTI-MEDIA FILE SYSTEM BACKEND HOOK
  uploadChatAttachment: async (file: File) => {
    const formData = new FormData();
    formData.append("file", file);

    const targetUrl = API_ROUTES.COMPANY.CHATS.CONVERSATIONS.replace(
      "conversations",
      "upload-attachment",
    );
    const response = await api.post<any>(targetUrl, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    return response.data;
  },
  ...
  leaveConversation: async (id: string) => {
    const response = await api.post(
      `${API_ROUTES.COMPANY.CHATS.CONVERSATIONS}/${id}/leave`,
    );
    return response.data;
  },
  ...
  editMessage: async (messageId: string, content: string) => {
    const response = await api.patch(
      `${API_ROUTES.COMPANY.CHATS.BASE}/messages/${messageId}`,
      { content },
    );
    return response.data;
  },
  ...
  deleteMessage: async (messageId: string, type: "me" | "everyone") => {
    const response = await api.delete(
      `${API_ROUTES.COMPANY.CHATS.BASE}/messages/${messageId}?type=${type}`,
    );
    return response.data;
  },
};
